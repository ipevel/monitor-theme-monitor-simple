...
`npm test` 钉住的是那些「改坏了界面不报错、只会画错」的地方：`src/lib/format.ts` 的刻度阶梯、`src/lib/series.ts` 的削峰、`src/lib/severity.ts` 的告警阈值、`src/lib/node.ts` 的负载/交换判定与陈旧判断（这里错了，一台被 swap 打爆的机器会安静地留在灰色里），以及 `src/assets/flags.ts` 那串国旗 sprite 的内部一致性。
