...
- 排序：问题优先（默认）/ CPU 占用 / 内存占用 / 本月流量 / 到期时间
  ...
  - 手动切换排序时卡片以滑动动画归位（View Transition），减少跳动的视觉噪声；系统开了「减少动态效果」时直接就位
- 搜索名称或地区
- 网格 / 列表视图
- **网络质量**开关（默认关闭）：打开后每张卡多一行延迟与丢包，每分钟向各节点的历史接口取一次
- 筛选条件写进 URL（`?country=JP&status=离线&sort=cpu&q=…`），可以直接分享，刷新也不丢
- 节点卡：名称 + 国旗 + 状态，三个大数字（CPU / 内存 / 硬盘，各带细进度条），一行脚注（本月流量 / 到期 / 流量重置）
  - 80% 转琥珀、92% 转红，低于阈值一律不着色；**告警时卡片左侧多一道色条**，四列白色卡片里一眼能扫到是哪几张
  - **负载与交换**另起一行，各带半高小进度条：CPU 是两秒的窗口，一台 load 跑到核数二十倍的机器可能刚好落在调度间隙里读成 8%，而 swap 在被打爆时和正常机器长得一样 —— 两个数都在 payload 里，按同一套阈值着色；条的长度让饱和度先于颜色被看到
  - **本月流量**带配额的套餐在脚注上方画一条用量条（无限套餐没有分母，保持一行文字）；**实时速率**在流量跑动时比静止时亮一档，静止的机器保持安静灰
...
| `expires_in` 到期天数（按 hub 的日历） | v1.3.0 | 按 `expires_at` 自己算（`daysUntil`，访客本地时区） |

`month_used` 在时以 hub 的为准：面板、流量告警和通知读的都是它，主题自己再算一遍，
迟早会和告警报出来的数不一样。

`expires_in` 同理，只是多一层：`expires_at` 是一个日期，每个访客用自己的时钟去读，
时区不同读出的天数就不同 —— 访客比 hub 快时，一台还没到期的机器会提前一天显示
「已过期」。天数由 hub 按自己的日历数，主题只在 hub 没给时才自己算。
